...
#define BOUNDARY_GRAPH_APPROX 1.5
#define SQRT_APPROX 1.5
#define APPROX 1.5
...
struct CSP{
    ...
    void load_directed_graph(string graph_name){
        ...
        for(; fscanf(file, "%d:", &u)==1; lines++){
            for(; fscanf(file, "%d", &v)==1;){
                ...
                link_list_edge forward_tri(v-1, weight, cost);  
                link_list_edge backward_tri(u-1, weight, cost);
                if(cost > max_cost) max_cost = cost;
                links[0][u-1].push_back(forward_tri);
                links[1][v-1].push_back(backward_tri);
    ...
    void load_directed_graph(string graph_name, string partition_name, string cut_name){
        ...
        for(; fscanf(file, "%d:", &u)==1; lines++){
            for(; fscanf(file, "%d", &v)==1;){
                ...
                link_list_edge backward_tri(u-1, weight, cost);
                if(cost > max_cost) max_cost = cost;
                original_links[0][u - 1].push_back(forward_tri);
                original_links[1][v - 1].push_back(backward_tri);
                tmp_cost=  M*1.0/(M+1) * tmp_cost +cost*1.0/(M+1);
                M++;
            }
        }
        ...
        contrat_file_name = "bg."+contrat_file_name;
    ...
    void load_query( string query_file_name){
        ...
        int u, v, cost_lower, cost_upper, cost_limit, dist_limit;
        int cnt=0;
        for(; fscanf(query_file, "%d %d %d %d %d %d\n", &u, &v, &cost_lower, 
                     &cost_upper, &cost_limit, &dist_limit)==6;){
            query_set[cnt].u = u - 1;
            query_set[cnt].v = v - 1;
            //query_set[cnt].cost_limit = cost_limit+ 4*cost_upper;
            query_set[cnt].cost_limit = cost_limit;
            query_set[cnt].dist_limit = dist_limit;
            cnt++;
        }
        fclose(query_file);
    }
    ...
    vector<edge_list > constraint_shortest_path_query(int u, int v, int cost){
        ...
        if(partition[u] == partition[v]){
            cout<<"the same partition\n";
            c1++;
            path = dijkstra(u, v, cost);
            return path;
        }else{
            cout<<"different partition\n";
            c2++;
    ...
    void output_csp(string filename){
        string full_name = filename+".index";
        string order_name = filename+".order";
...
int main(int argc, char** argv){
    ...
    string Output_FileName = "COLA_output";
    ...
    if(do_query_test){
       ...
       for(int i=0; i< num_of_queries ; i++){
           ...
           for(int tries=0; tries< 10; tries++){
               pathes[tries] = csp.constraint_shortest_path_query(csp.query_set[i].u, csp.query_set[i].v, csp.query_set[i].cost_limit);
               if(pathes[tries].size()>0){
                   reach = true;
                   whichtry = tries;
               }
           }
           ...
           path = pathes[whichtry];
           int temp_pathlength = 0;
           if(!path.empty())
           {
               for(int j=0; j<path.size();++j)
               {
                   const edge_list& e = path[j];
                   printf("%d -> %d (%d %d)", e.u, e.v, e.weight, e.cost);
                   temp_pathlength += e.weight;
               }
               if (temp_pathlength < csp.query_set[i].dist_limit){
                temp_pathlength = csp.query_set[i].dist_limit;
               }
               TOTAL_edgelength += temp_pathlength;
               printf("\n");
           }
       }
       //cout<<csp.c1<<endl;
       //cout<<csp.c2<<endl;
        ...
        ofstream of;
        of.open(Output_FileName,ofstream::app);

        of << "Total queries: " << num_of_queries << endl;

       //of<<"reachable_queries: "<<csp.reachable_queries<<endl;
       of<<"reachable_cnt: "<<reachable_cnt<<endl;
       //of<<"unfold_shortcut_num: "<<csp.unfold_shortcut_num<<endl;
       //of<<"unfold_edge_num: "<<csp.unfold_edge_num<<endl;
       end_q = clock();
       double Query_Time = (end_q-start_q)/double(CLOCKS_PER_SEC);
       of << "Query Time: " << Query_Time << endl;

       double AVG_Time = Query_Time/double(num_of_queries);
       of << "Average Time: " << AVG_Time << endl;

       double AVG_pathlength = TOTAL_edgelength/double(reachable_cnt);
       of << "Average Path Length: " << AVG_pathlength << endl;
       of << endl;
   }
