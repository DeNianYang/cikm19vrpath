    ...
    for i in range(w):
        for j in range(w):
            if mtx[i][j] == 'O':
                cell_sum += 1
    #print mtx, cell_sum
    if cell_sum < 7 and ((mtx[0][1] == 'O' and  mtx[2][1] == 'O') or (mtx[1][0] == 'O' and mtx[1][2] == 'O')):
        return ' '
    return 'R'
...
def detect(mtx, pois):
    ...
    for x in range( 1, width-1 ):
        for y in range( 1, width-1 ):
            mtx[x][y] = detect_cell( [i[y-1:y+2] for i in mtx[x-1:x+2]])
            if mtx[x][y] == 'R':
                ref_num += 1
            if mtx[x][y] == ' ':
    ...
    shuffle(empty_list)
    if ref_num < pois:
        for i in range( min(pois-ref_num, len(empty_list)) ):
            x = empty_list[i][0]
            y = empty_list[i][1]
            mtx[x][y] = 'P'


def maze_to_file(mtx, file_name):
...
if __name__ == '__main__':
    ...
    m = Maze.generate(width, width)
    m_str = m._to_str_matrix()

    n = copy.deepcopy(m_str)
    detect(n, pois)
    maze_to_file(n, file_name)

    debug_display(m_str, n)

    #import console_maze as console
    #try:
    #    while MazeGame(Maze.generate(width, height)).play(): pass
    #except:
    #    import traceback
    #    traceback.print_exc(file=open('error_log.txt', 'w'))
